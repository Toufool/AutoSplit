...
# gapi off: its serialization needs winsock (htonl/ntohl), but the sdist passes a
# malformed `/lws2_32` flag to MSVC LINK, so ws2_32 is never linked -> LNK2019.
$Env:CMAKE_ARGS = '-DBUILD_opencv_dnn=OFF -DENABLE_NEON=OFF -DBUILD_opencv_gapi=OFF'
