...
class HotKeyDef:
    ...
    def set_sequence(self, sequence_str: str):
        ...
        for elem in split:
            ...
            assert_and_show_error(self.autosplit, elem in STR_TO_KEYS, f"unsupported key: {elem!r}")
...
class HotKeyThread(QThread):
        ...
        if hotkey == "toggle_auto_reset_image":
            ...
            def toggle_auto_reset_image():
                ...
                if self.autosplit.SettingsWidget:
                    self.autosplit.SettingsWidget.enable_auto_reset_image_checkbox.setChecked(
                        new_value
                    )
    ...
    def run_action_from_cmd(self, cmd: CommandStr):
        ...
        assert_and_show_error(
            self.autosplit, len(keydef.send_keys) > 0, "can't send inputs: sequence is not set"
        )
