...
if sys.version_info >= (3, 15):
    # Our own top-level modules and packages, generated by compile_resources.ps1
    # because PyInstaller-frozen builds can't discover pure modules on disk
    # (they live inside the PYZ archive).
    from gen.build_vars import SRC_ROOT_MODULES
    ...
    def _lazy_imports_filter(
        ...
        # Only our own imports are lazy. Third-party internals must stay eager:
        # shiboken6's exec'd signature bootstrap can't resolve lazy proxies
        # (and hides behind aliased module names), and numpy's import-order
        # self-check raises a bogus version conflict.
        if not importer:
            return False
        # A package importing its own submodule must stay eager: the import
        # system rebinds the submodule name on the parent package, clobbering
        # any same-named lazy proxy in the package's namespace
        ...
        if imported.startswith(importer + "."):
            return False
        return importer.partition(".")[0] in SRC_ROOT_MODULES
